		...
		*/
		...
		function apply(ctx) {
			...
			const saveConfiguration = async (settings) => {
				...
				if (current.status !== "ready" || current.value === void 0 || current.revision === void 0 || !current.writable) throw new Error(t("saveFailed"));
				...
				if (!result.ok) throw new Error(result.error.message);
				if (!await form.mutate(ops, current.revision)) throw new Error(t("saveFailed"));
