window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const COMMANDCODE_VALIDATE_ENDPOINT = "settings/validate";
		...
		//#endregion
		//#region node_modules/.pnpm/dsh-llm-providers-ui@https+++github.com+NOirBRight+dsh-llm-providers-ui+releases+downlo_f24ab602d7dec4a0f05f33177a4f1e14/node_modules/dsh-llm-providers-ui/lib/usage-readers.js
		...
		*/
		...
		function commandCodeMonthlyCap(planId) {
			...
			return COMMAND_CODE_MONTHLY_USD.find(([key]) => id.startsWith(key) || id.includes(key))?.[1];
		}
		function decodeCommandCodeUsage(usage) {
			...
			const value = recordUsageValue$1(credits);
			if (value === void 0) return void 0;
			const windows = [];
			const monthly = value.monthlyCredits;
			const plan = recordUsageValue$1(usage.plan);
			const planId = plan !== void 0 && nonEmptyString$1(plan.planId) ? plan.planId : void 0;
			for (const [key, label] of [["fiveHour", "5-hour"], ["weekly", "Week"]]) {
				const raw = value[key];
				if (raw === void 0) continue;
				const item = recordUsageValue$1(raw);
				if (item === void 0 || !nonNegativeNumber$1(item.used) || !nonNegativeNumber$1(item.cap)) return void 0;
				const resetAt = nonEmptyString$1(item.resetAt) ? item.resetAt : void 0;
				if (item.resetAt !== void 0 && resetAt === void 0) return void 0;
				windows.push(remainingWindow({
					id: key,
					label,
					used: item.used,
					limit: item.cap,
					...resetAt === void 0 ? {} : { resetsAt: resetAt }
				}));
			}
			if (monthly !== void 0) {
				if (!nonNegativeNumber$1(monthly)) return void 0;
				const cap = commandCodeMonthlyCap(planId);
				if (cap !== void 0 && cap > 0 && monthly <= cap) {
					const periodEnd = plan !== void 0 && nonEmptyString$1(plan.currentPeriodEnd) ? plan.currentPeriodEnd : void 0;
					windows.push(remainingWindow({
						id: "monthly",
						label: "Month",
						used: cap - monthly,
						limit: cap,
						...periodEnd === void 0 ? {} : { resetsAt: periodEnd }
					}));
				}
			}
			return {
		...
		*/
		...
		//#endregion
		//#region node_modules/.pnpm/dsh-llm-providers-ui@https+++github.com+NOirBRight+dsh-llm-providers-ui+releases+downlo_f24ab602d7dec4a0f05f33177a4f1e14/node_modules/dsh-llm-providers-ui/lib/provider-ui.js
		/** Plain-object guard shared by the reader factories and the sidebar cache validator. */
		...
		*/
		...
		//#endregion
		//#region node_modules/.pnpm/dsh-llm-providers-ui@https+++github.com+NOirBRight+dsh-llm-providers-ui+releases+downlo_f24ab602d7dec4a0f05f33177a4f1e14/node_modules/dsh-llm-providers-ui/lib/sortable.js
		...
		*/
		function SortableList({ items, getId, renderItem, dragLabel, onReorder, disabled = false, chrome = "row", sorting = true, moveButtons = false, moveUpLabel, moveDownLabel }) {
			...
			const rowChromeStyle = bare ? bareRowStyle : plain ? plainRowStyle : card ? cardRowStyle : rowStyle$1;
		...
		*/
			...
			return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
				...
				children: [
					...
					/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
						...
						}), visionLabel] }), /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("label", { children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("input", {
							type: "checkbox",
		...
		function CommandCodeSettingsCard(props) {
			...
			const loadUsage = async () => {
				...
				if (draft === void 0 || snapshot.value?.usageEnabled === false || credential?.configured !== true) return;
			...
			const modelsList = /* @__PURE__ */ (0, react_jsx_runtime.jsxs)(react_jsx_runtime.Fragment, { children: [
			...
			const advancedBlock = /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
				className: "c-control",
				children: [/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("label", {
					...
					children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("input", {
						...
						disabled,
						onChange: (event) => {
							patchDraft({ zeroDataRetention: event.target.checked });
							setNotice(void 0);
						}
					}), t("zdr")]
				}), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("p", {
			...
			const modelExtra = (item, index) => /* @__PURE__ */ (0, react_jsx_runtime.jsx)(ModelDetails, {
				model: item,
				disabled,
				t,
				...
				children: [
					...
					/* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
						...
						children: /* @__PURE__ */ (0, react_jsx_runtime.jsx)(ProviderCardHeader, {
							...
							role: "llm",
							...headerQuota === null ? credential?.configured === true && (usage.status === "error" || usage.status === "unsupported") ? { quota: { label: t("quota") } } : {} : { quota: headerQuota }
						})
					}),
		...
		*/
		...
		function apply(ctx) {
			...
			const publishAccount = (state) => {
				...
				account.state = state;
				try {
					ctx.get("providerDirectory")?.update(COMMANDCODE_SETTINGS_NAMESPACE);
				} catch {}
			};
			const form = ctx.configForms.get(COMMANDCODE_SETTINGS_NAMESPACE);
			const { rpc } = ctx.get("connection");
			const callPlugin = (endpoint, payload, signal) => rpc.call(COMMANDCODE_RPC_CHANNEL, COMMANDCODE_RPC_METHOD, {
				endpoint,
				payload
			}, signal);
			...
			const storeApiKey = async (value) => {
				...
				if (!result.ok) throw new Error(result.error.message);
				dropPersistedUsageKeys$1([COMMANDCODE_SETTINGS_NAMESPACE]);
			...
			const saveConfiguration = async (settings, sourceRevision) => {
				...
				if (current.status !== "ready" || current.value === void 0 || current.revision === void 0 || !current.writable || current.revision !== sourceRevision) throw new Error(t("saveFailed"));
				const before = decodeCommandCodeSettings(current.value);
				if (before === void 0) throw new Error(t("saveFailed"));
				const ops = [];
				for (const field of [
					"models",
					...
					ops.push({
						op: "set",
						path: [field],
						value: field === "models" ? JSON.parse(JSON.stringify(settings.models)) : settings[field]
					});
				}
				...
				if (!result.ok) throw new Error(result.error.message);
				if (!await form.mutate(ops, sourceRevision)) throw new Error(t("saveFailed"));
				const saved = form.getSnapshot();
				const value = decodeCommandCodeSettings(saved.value);
				if (value === void 0 || saved.revision === void 0) throw new Error(t("saveFailed"));
				return {
					settings: value,
			...
			const fetchUsage = async () => {
				...
				if (!result.ok) {
					if (result.error.code === "INVALID_CREDENTIAL") dropPersistedUsageKeys$1([COMMANDCODE_SETTINGS_NAMESPACE]);
			...
			ctx.inject(["providerDirectory"], (providerCtx) => {
				providerCtx.effect(() => {
					...
					return providerCtx.providerDirectory.register(declaration);
				}, "dsh-llm-commandcode: provider directory");
			});
			ctx.effect(() => {
				describeCredential().catch(() => {
					publishAccount("unknown");
				});
				return () => {
					closed = true;
				};
