...
function decodeCommandCodeModel(value) {
	if (!record$2(value) || typeof value.id !== "string" || value.id.length === 0) return void 0;
...
function decodeCommandCodeDiscoveryResult(value) {
	if (!record$2(value) || !Array.isArray(value.models) || !Array.isArray(value.warnings)) return void 0;
	const models = [];
...
function decodeCommandCodeUsageView(value) {
	if (!record$2(value) || typeof value.fetchedAt !== "string" || !Array.isArray(value.failures)) return void 0;
...
function toCommandCodePiAiModel(model, connection) {
	...
	return {
		...
		input: model.inputModalities === void 0 ? ["text"] : [...model.inputModalities],
		cost: NO_COST,
...
function resolveModels(models) {
	...
	return [...models ?? DEFAULT_MODELS].map((model) => {
		...
		if (model.defaultEffort !== void 0 && !efforts.includes(model.defaultEffort)) throw new Error("llm-commandcode: defaultEffort is not offered for " + model.id);
		const input = model.inputModalities === void 0 || model.inputModalities.length === 0 ? ["text"] : model.inputModalities;
...
function apply(ctx, config) {
	...
	ctx.llm.registerModelDiscovery(NS, async (request) => {
		return (await discoverModels({ ...request.signal === void 0 ? {} : { signal: request.signal } })).models;
	});
	ctx.inject(["connection"], (connectionCtx) => {
		connectionCtx.connection.rpc.handle(COMMANDCODE_RPC_CHANNEL, async (endpoint, payload, signal) => {
...
//#endregion
export { COMMANDCODE_PROVIDER, COMMANDCODE_RPC_CHANNEL, COMMANDCODE_SETTINGS_NAMESPACE, CommandCodeAdapter, Config, DEFAULT_API_KEY_ENV, DEFAULT_CONTEXT_WINDOW, DEFAULT_MAX_TOKENS, DEFAULT_MODELS, DEFAULT_REQUEST_TIMEOUT_MS, DEFAULT_STREAM_IDLE_TIMEOUT_MS, PUBLIC_PROVIDER_BASE_URL, apply, decodeCommandCodeDiscoveryRequest, decodeCommandCodeDiscoveryResult, decodeCommandCodeModel, decodeCommandCodeSaveRequest, decodeCommandCodeSaveResult, decodeCommandCodeSettings, decodeCommandCodeUsageReply, decodeCommandCodeUsageRequest, decodeCommandCodeUsageView, discoverModels, inject, name, parseCommandCodeModels, parseCommandCodeUsageBodies, protocolForModel, resolveAdapterOptions };
