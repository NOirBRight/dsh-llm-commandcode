...
const COMMANDCODE_RPC_CHANNEL = "/api";
const COMMANDCODE_RPC_METHOD = "plugin-rpc/commandcode";
...
function decodeCommandCodeModel(value) {
	if (!record$3(value) || hasTokenFields(value) || typeof value.id !== "string" || value.id.length === 0) return void 0;
...
function decodeCommandCodeSettings(value) {
	if (!record$3(value) || hasTokenFields(value)) return void 0;
...
function decodeCommandCodeDiscoveryRequest(value) {
	if (!record$3(value) || Object.keys(value).length !== 0) return void 0;
	return {};
}
function decodeCommandCodeDiscoveryResult(value) {
	if (!record$3(value) || hasTokenFields(value) || !Array.isArray(value.models) || !Array.isArray(value.warnings)) return void 0;
	const models = [];
...
function decodeCommandCodeValidateRequest(value) {
	if (!record$3(value) || hasTokenFields(value) || Object.keys(value).some((key) => key !== "settings")) return void 0;
...
function decodeUsageWindow(value) {
	if (!record$3(value) || !positiveOrZero(value.used) || !positiveOrZero(value.cap)) return void 0;
...
function decodeCommandCodeCredentialSetRequest(value) {
	if (!record$3(value) || Object.keys(value).some((key) => key !== "apiKey") || typeof value.apiKey !== "string" || value.apiKey.trim().length === 0) return void 0;
	return { apiKey: value.apiKey };
}
function decodeCommandCodeUsageView(value) {
	if (!record$3(value) || hasTokenFields(value) || typeof value.fetchedAt !== "string" || !Array.isArray(value.failures)) return void 0;
	...
	if (value.account !== void 0) {
		if (!record$3(value.account)) return void 0;
	...
	if (value.credits !== void 0) {
		if (!record$3(value.credits)) return void 0;
	...
	if (value.plan !== void 0) {
		if (!record$3(value.plan)) return void 0;
		for (const key of [
			"planId",
			"name",
			"status",
	...
	if (value.summary !== void 0) {
		if (!record$3(value.summary)) return void 0;
		for (const key of [
			"totalCost",
			"totalTokensIn",
			"totalTokensOut",
...
function decodeCommandCodeUsageReply(value) {
	if (!record$3(value) || value.status !== "ok" && value.status !== "unsupported") return void 0;
...
function hydrateFromDisk() {
	...
	try {
		...
		if (!record$2(parsed) || typeof parsed.at !== "number" || !Array.isArray(parsed.models)) return;
...
function peekCommandCodeModelsDev() {
	hydrateFromDisk();
	return cache?.overlay;
}
function effortValues(value) {
	if (!record$2(value)) return [];
	const options = value.reasoning_options;
	if (!Array.isArray(options)) return [];
	const found = [];
	for (const option of options) {
		if (!record$2(option) || !Array.isArray(option.values)) continue;
...
function visionOf(value) {
	const modalities = record$2(value.modalities) ? value.modalities.input : void 0;
	if (!Array.isArray(modalities)) return void 0;
	return modalities.includes("image");
}
function rowScore(providerId, value) {
...
function parseCommandCodeModelsDevRow(id, value) {
	if (!record$2(value)) return void 0;
	const limit = record$2(value.limit) ? value.limit : void 0;
	...
	return {
		...
		...vision === true ? { inputModalities: ["text", "image"] } : vision === false ? { inputModalities: ["text"] } : {},
		...thinking === void 0 ? {} : { thinking },
...
function parseCommandCodeModelsDev(value) {
	if (!record$2(value)) return /* @__PURE__ */ new Map();
	const scored = /* @__PURE__ */ new Map();
	for (const [providerId, provider] of Object.entries(value)) {
		const models = record$2(provider) ? provider.models : void 0;
		if (!record$2(models)) continue;
		for (const [id, row] of Object.entries(models)) {
			if (id.length === 0 || !record$2(row)) continue;
			const parsed = parseCommandCodeModelsDevRow(id, row);
			if (parsed === void 0) continue;
			const key = id.toLowerCase();
			const score = rowScore(providerId, row);
			const prev = scored.get(key);
			if (prev === void 0 || score > prev.score) scored.set(key, {
				score,
				model: parsed
			});
		}
...
async function fetchAndStore(fetchImpl, signal) {
	...
	try {
		...
		const overlay = parseCommandCodeModelsDev(JSON.parse(await readBoundedText(response, MODELS_DEV_MAX_BYTES, MODELS_DEV_URL, "DISCOVERY_FAILED", requestSignal)));
		const at = Date.now();
		cache = {
			at,
			overlay
...
async function loadCommandCodeModelsDev(fetchImpl = fetch, signal, options) {
	hydrateFromDisk();
	const now = Date.now();
...
function hasCommandCodeCapabilitySnapshot(id) {
	return effortsForCommandCodeModel({ id }).length > 0 || hasNativeReasoningByDefault(id) || inputModalitiesForCommandCodeModel(id).includes("image");
}
/** Parse the provider's OpenAI-shaped model list without inventing capacity. */
function parseCommandCodeModels(value, overlay = /* @__PURE__ */ new Map()) {
	...
	for (const raw of data) {
		...
		const inputModalities = snapshotImage || overlayImage ? ["text", "image"] : ["text"];
		const thinkingEfforts = effortsForCommandCodeModel({ id }).length > 0 ? void 0 : extra?.thinkingEfforts;
		const defaultEffort = defaultEffortForCommandCodeModel(thinkingEfforts === void 0 || thinkingEfforts.length === 0 ? { id } : {
			id,
			thinkingEfforts
		});
		const thinking = hasNativeReasoningByDefault(id) || extra?.thinking === true ? true : void 0;
		models.push({
			id,
			...name === void 0 ? {} : { name },
			...
			...defaultEffort === void 0 ? {} : { defaultEffort },
			...thinking === void 0 ? {} : { thinking },
			...thinkingEfforts === void 0 || thinkingEfforts.length === 0 ? {} : { thinkingEfforts },
			inputModalities
		});
	}
	return {
...
async function overlayForListing(fetchImpl) {
	...
	return await Promise.race([pending, new Promise((resolve) => {
		budget.addEventListener("abort", () => {
			resolve(peekCommandCodeModelsDev() ?? /* @__PURE__ */ new Map());
		}, { once: true });
	})]);
}
...
async function readCommandCodeUsage(request, resolveCredential, fetchImpl = fetch) {
	...
	const get = async (path) => {
		...
		if (result.status === 401 || result.status === 403) throw new LlmError(path + ": HTTP " + String(result.status) + "; the Command Code API key is unusable", INVALID_CREDENTIAL_CODE);
...
function normalizeThinkingEfforts(values) {
	if (values === void 0 || values.length === 0) return void 0;
	const out = [];
	for (const value of values) {
		const effort = canonCommandCodeEffort(value);
		if (effort === void 0) throw new Error("llm-commandcode: invalid thinkingEfforts token " + value);
...
function resolveModels(models) {
	...
	return [...models ?? DEFAULT_MODELS].map((model) => {
		...
		const efforts = effortsForCommandCodeModel({
			id: model.id,
			...normalizedEffort === void 0 ? {} : { defaultEffort: normalizedEffort },
		...
		const offeredEffort = normalizedEffort !== void 0 && efforts.includes(normalizedEffort) ? normalizedEffort : void 0;
		const defaultEffort = model.thinking === false ? void 0 : defaultEffortForCommandCodeModel({
			id: model.id,
			...offeredEffort === void 0 ? {} : { defaultEffort: offeredEffort },
			...overlayEfforts === void 0 ? {} : { thinkingEfforts: overlayEfforts }
		});
		const thinkingEfforts = effortsForCommandCodeModel({ id: model.id }).length > 0 ? void 0 : overlayEfforts;
...
const CREDENTIAL_FAILURE_CODES = /* @__PURE__ */ new Set([INVALID_CREDENTIAL_CODE, "MISSING_CREDENTIAL"]);
/**
...
function decodePluginCall(value) {
	if (typeof value !== "object" || value === null || Array.isArray(value)) return void 0;
	const call = value;
	const keys = Object.keys(call);
	if (keys.length < 1 || keys.length > 2 || keys.some((key) => key !== "endpoint" && key !== "payload") || typeof call.endpoint !== "string") return void 0;
	return {
		endpoint: call.endpoint,
		payload: call.payload
	};
}
function pluginResponse(rpcId, result) {
...
function badPluginRequest(status) {
	return new Response(status === 415 ? "Unsupported Media Type" : "Bad Request", { status });
}
const COMMANDCODE_RPC_ROUTE = "/api/plugin-rpc/commandcode";
function resolveAdapterOptionsFromValues(config) {
...
function apply(ctx, config) {
	...
	ctx.on("loader/volatile-update", (paths) => {
		if (!paths.some(([field]) => field !== void 0 && VOLATILE_CONFIG_FIELDS[field] === true)) return;
	...
	const handleRpc = async (endpoint, payload, signal, _operator) => {
		if (endpoint === "settings/validate") {
			...
			try {
				resolveAdapterOptions(Config({
					...currentConfig.apiKeyEnv === void 0 ? {} : { apiKeyEnv: currentConfig.apiKeyEnv },
					...request.settings
				}));
				return {
					ok: true,
	...
	ctx.effect(() => {
		return ctx.inject(["connection", "webServer"], (connectionCtx) => {
			...
			connectionCtx.effect(() => connectionCtx.connection.fetch.register({
				...
				fetch: async (request) => {
					if (request.headers.get("content-type")?.split(";", 1)[0]?.trim().toLowerCase() !== "application/json") return badPluginRequest(415);
					let body;
					try {
					...
					if (!parsed.success || parsed.data.method !== "plugin-rpc/commandcode") return badPluginRequest(400);
					const call = decodePluginCall(parsed.data.payload);
					if (call === void 0) return badPluginRequest(400);
					try {
	...
	ctx.inject(["settings"], (settingsCtx) => {
		settingsCtx.effect(() => settingsCtx.settings.configure({ auto: false }, ctx.fiber), "llm-commandcode: settings presentation");
	});
}
//#endregion
export { COMMANDCODE_CREDENTIAL_SET_ENDPOINT, COMMANDCODE_CREDENTIAL_STATUS_ENDPOINT, COMMANDCODE_PROVIDER, COMMANDCODE_RPC_CHANNEL, COMMANDCODE_RPC_METHOD, COMMANDCODE_SETTINGS_NAMESPACE, CommandCodeAdapter, Config, DEFAULT_API_KEY_ENV, DEFAULT_CONTEXT_WINDOW, DEFAULT_MAX_TOKENS, DEFAULT_MODELS, DEFAULT_REQUEST_TIMEOUT_MS, DEFAULT_STREAM_IDLE_TIMEOUT_MS, PUBLIC_PROVIDER_BASE_URL, apply, decodeCommandCodeCredentialSetRequest, decodeCommandCodeDiscoveryRequest, decodeCommandCodeDiscoveryResult, decodeCommandCodeModel, decodeCommandCodeSettings, decodeCommandCodeUsageReply, decodeCommandCodeUsageRequest, decodeCommandCodeUsageView, decodeCommandCodeValidateRequest, discoverModels, inject, name, parseCommandCodeModels, parseCommandCodeUsageBodies, protocolForModel, resolveAdapterOptions, usageFailure };
