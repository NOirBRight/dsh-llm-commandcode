...
export type CommandCodeUsageRead =
  | { status: 'ok'; usage: CommandCodeUsageView }
  | { status: 'unsupported' }

/** Model metadata used by the adapter's public list. */
export function modelInfo(provider: string, model: CommandCodeModelConfig): LlmModelInfo {
  return {
    provider,
    id: model.id,
...
export function effectiveContext(model: CommandCodeModelConfig, fallback: number): number {
  return model.contextWindowOverride ?? model.contextWindow ?? fallback
}

/** Resolve the model's wire protocol. */
export function effectiveApi(model: Pick<CommandCodeModelConfig, 'id'>): CommandCodeApi {
